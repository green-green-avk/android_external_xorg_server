...
static void
fakeInternalDamageRedisplay(ScreenPtr pScreen)
{
    KdScreenPriv(pScreen);
    KdScreenInfo *screen = pScreenPriv->screen;
    FakePriv *scrpriv = screen->driver;
    RegionPtr pRegion;
...
static void
fakeScreenBlockHandler(ScreenPtr pScreen, void *timeout)
{
    KdScreenPriv(pScreen);
    KdScreenInfo *screen = pScreenPriv->screen;
    FakePriv *scrpriv = screen->driver;
    ...
    pScreen->BlockHandler = fakeScreenBlockHandler;

    if (scrpriv->pDamage)
        fakeInternalDamageRedisplay(pScreen);
}

Bool
fakeSetInternalDamage(ScreenPtr pScreen)
{
    KdScreenPriv(pScreen);
    KdScreenInfo *screen = pScreenPriv->screen;
    FakePriv *scrpriv = screen->driver;
...
void
fakeUnsetInternalDamage(ScreenPtr pScreen)
{
    KdScreenPriv(pScreen);
    KdScreenInfo *screen = pScreenPriv->screen;
    FakePriv *scrpriv = screen->driver;
...
void
fakeCloseScreen(ScreenPtr pScreen) {
	fakeUnsetInternalDamage(pScreen);
}
...
fakeCreateResources(ScreenPtr pScreen)
{
    ...
    return fakeSetInternalDamage(pScreen);
}
