...
static struct jniMethods {
	jniMethod force_finish;
} jniMethods;
...
static void prepare(void) {
	...
	jniMethods.force_finish.class = (*env)->GetObjectClass(env, android_app_state->activity->clazz);
    jniMethods.force_finish.method = (*env)->GetMethodID(env, classActivity, "finish", "()V");
...
static void activity_force_finish(void) {
    ...
    (*vm)->AttachCurrentThread(vm, &env, 0);
    (*env)->CallVoidMethod(env, jniMethods.force_finish.class, jniMethods.force_finish.method);
