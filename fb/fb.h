...
#define FbSelectPatternPart(c_xor,o,t)	((c_xor) >> (FbPatternOffset (o,t) << 3))
#define FbStorePart(dst,off,t,c_xor)	(WRITE(FbPtrOffset(dst,off,t), \
...
#define FbDoLeftMaskByteRRop(dst,lb,l,c_and,c_xor) { \
	...
	break; \
    default: \
	WRITE(dst, FbDoMaskRRop(READ(dst), c_and, c_xor, l)); \
	break; \
    } \
}
...
#define FbDoRightMaskByteRRop(dst,rb,r,c_and,c_xor) { \
	...
	break; \
    default: \
	WRITE(dst, FbDoMaskRRop (READ(dst), c_and, c_xor, r)); \
    } \
}
...
fbDots8(FbBits * dst,
        ...
        xPoint * pts,
        int npt,
        int xorg, int yorg, int xoff, int yoff, FbBits c_and, FbBits c_xor);

extern _X_EXPORT void

fbArc8(FbBits * dst,
       FbStride dstStride,
       int dstBpp, xArc * arc, int dx, int dy, FbBits c_and, FbBits c_xor);
...
fbSolidBoxClipped(DrawablePtr pDrawable,
                  RegionPtr pClip,
                  int xa, int ya, int xb, int yb, FbBits c_and, FbBits c_xor);
...
fbSolid(FbBits * dst,
        FbStride dstStride,
        int dstX, int bpp, int width, int height, FbBits c_and, FbBits c_xor);
...
fbFillRegionSolid(DrawablePtr pDrawable,
                  RegionPtr pRegion, FbBits c_and, FbBits c_xor);
